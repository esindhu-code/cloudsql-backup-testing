...
ALERT_TOPIC = "projects/your-project-id/topics/process-pubsub-message"  # Pub/Sub alert topic
...
def initiate_backup(instance_id):
    ...
    try:
        ...
        backup_description = f"Automated backup - {datetime.now().strftime('%b %d, %Y, %I:%M:%S %p')}"
        request = service.backupRuns().insert(
            project=PROJECT_ID,
            instance=instance_id,
            body={"description": backup_description}
        )
